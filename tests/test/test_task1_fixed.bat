...
javac -d %BIN_DIR% -cp %CLASSPATH% core\Character.java core\Student.java 2^>errors.txt
...
javac -d %BIN_DIR% -cp "%CLASSPATH%;%BIN_DIR%" tests\Task1StudentTests.java 2^>errors.txt

if errorlevel 1 (
    echo.
    echo [X] ERROR: Test won't compile!
    echo.
    echo You're probably missing these methods in Student.java:
    echo   - increaseEnergy(int amount)
    echo   - decreaseEnergy(int amount)
    echo   - increaseKnowledge(int amount)
    echo   - getKnowledge()
    echo   - getEnergy()
    echo.
