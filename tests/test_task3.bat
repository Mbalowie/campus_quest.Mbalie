...
javac -d %BIN_DIR% -cp %CLASSPATH% ^
  ...
  ..\events\RandomEncounter.java 2> errors.txt
...
javac -d %BIN_DIR% -cp "%CLASSPATH%;%BIN_DIR%" Task3EventTests.java 2> errors.txt

if errorlevel 1 (
    echo.
    echo [X] ERROR: Test won't compile!
    echo.
    echo You're probably missing the perform method in:
